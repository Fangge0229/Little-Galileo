...
struct SkyMapView: View {
    ...
    private func asterismPreview(_ asterism: Asterism, compact: Bool) -> some View {
        let unitPoints = previewUnitPoints(for: asterism)
        let lineColor = compact ? Color(hex: "4AC6D9").opacity(0.78) : Color(hex: "4A90D9").opacity(0.82)
        let starColor = compact ? Color(hex: "FFF8E7").opacity(0.92) : Color(hex: "FFD700").opacity(0.95)
        let background = compact ? Color.white.opacity(0.08) : Color.black.opacity(0.18)
        ...
        return Canvas { context, size in
            ...
            for hip in asterism.stars {
                ...
                let radius: CGFloat = compact ? 1.5 : 2.3
                let rect = CGRect(
                    x: center.x - radius,
                    y: center.y - radius,
                    width: radius * 2,
    ...
    private func previewUnitPoints(for asterism: Asterism) -> [Int: CGPoint] {
